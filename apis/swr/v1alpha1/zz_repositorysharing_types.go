...
type RepositorySharingInitParameters struct {
	...
	SharingAccount *string `json:"sharingAccount,omitempty" tf:"sharing_account,omitempty"`
}
...
type RepositorySharingSpec struct {
	...
	// autoscaler.
	InitProvider RepositorySharingInitParameters `json:"initProvider,omitempty"`
}
...
// RepositorySharing is the Schema for the RepositorySharings API. ""
...
type RepositorySharing struct {
	...
	// +kubebuilder:validation:XValidation:rule="!('*' in self.managementPolicies || 'Create' in self.managementPolicies || 'Update' in self.managementPolicies) || has(self.forProvider.sharingAccount) || (has(self.initProvider) && has(self.initProvider.sharingAccount))",message="spec.forProvider.sharingAccount is a required parameter"
